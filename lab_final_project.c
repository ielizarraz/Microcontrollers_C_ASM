...
void DS3231_Clear_Alarm_Int_Flag(char);
void Do_Setup_Time();
void Do_LED_Out(char);
...
#define Chip_Select     PORTCbits.RC1
#define heater_on_led   PORTCbits.RC0
#define Alarm_Alert     PORTBbits.RB1
...
unsigned char cnt, second, minute, hour, dow, day, month, year, old_second, alarm_sw_saved;
...
{
    INTCONbits.INT0IF = 0;
    DS3231_Clear_Alarm_Int_Flag(DS3231_ID);
    alarm_clear_flag = 1;
}
...
void INT2_isr(void)
{
   ...
   case 4:                                          // If nec_state is 4
        if ((timer >= 400) && (timer <= 1800))      // check if timer1 is between 400usec and 700usec
        {
            ...
            cnt++;                            // Increment bit_count
            if (cnt > 31)                     // Check if bit_count is greater than 31
            {           
...
void main()
{
    ...
    PORTD = 0;
     ...
     while(1)
    {   
         //printf("Alarm match = %x" , alarm_match_flag);
        ...
        if (second != old_second)
        {
            ...
            if ((DS3231_tempF < set_heater_temp) & heater_mode) heater_on_led = ON;
            else heater_on_led = OFF;

            DS3231_tempC = DS3231_Read_Temp(DS3231_ID);                
            Do_Update_Main_Screen();
            if (alarm_on_flag) {Do_RGB();}
    ...
    while (Setup_Mode == 1)
    {
        ...
        { 
            ...
            if (Key_Dn_Flag == 1 )
            {
                ...
                if (setup_heater_temp == 0 || setup_heater_temp == -1) setup_heater_temp = 99;
...
void DS3231_Turn_On_Alarm(char device)
{
  ...
  control = control | 0x01;
  I2C_Write_Address_Write_One_Byte(device, DS3231_Add_0E, control);
  control = I2C_Write_Address_Read_One_Byte(device, DS3231_Add_0F);
  control = control &0xfe;
  I2C_Write_Address_Write_One_Byte(device, DS3231_Add_0F, control);
...
void Do_RGB()
{
    ...
    if (ree == 7) { ree = 0;}
    ree++;
    char i3[] = {1 , 2 , 3, 4, 5 , 6 ,7};
    PORTD= i3[ree];
    
}
void DS3231_Clear_Alarm_Int_Flag(char device)
{
    char control;
    control = I2C_Write_Address_Read_One_Byte(device, DS3231_Add_0F);
    control = control & 0xFE;
