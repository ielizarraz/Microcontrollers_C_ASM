...
void Do_init()
{
     ...
     ADCON1 = 0x0F;                                     // Set ADCON1 with 0F
     TRISC=0x00;                                        // Setup port C with output
     TRISDbits.TRISD0 = 0;                              // Set zeroth bit of PORT D to output
     TRISEbits.TRISE0 = 0;                              // Set zeroth bit of PORT E to output
     SSPSTAT=0x40;                                      // SMP:
     ...
     T0CONbits.TMR0ON=1;                                // Turn on Timer 0 
      for(I = 0; I < 60; I++)                           // Looping through all array variables
     {
         if(I < 30)                                     // Set first half of array to low
         {
             Array[I] = 0x00; 
         }
         else
         {
             Array[I] = 0xFF;                           // Set second half of array to high
         }
     }
}
